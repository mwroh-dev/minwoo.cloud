...
- This repository is a single-language editorial site built with Next.js App Router and MDX content.
...
- `src/content/*.mdx` stores post frontmatter and body content.
- `src/lib/post.ts` parses frontmatter, derives href/reading time, and groups posts by series.
- `src/app/blog` and `src/app/blog/[slug]` render archive/detail routes from parsed post data.
...
- When changing routes, verify redirects, metadata, sitemap, and internal links together.
- When changing `src/content`, treat frontmatter validity and internal links as part of the code change.
